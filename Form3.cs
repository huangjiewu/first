...
namespace WindowsFormsApplication1
{
    public partial class Form3 : Form
    {
                    ...
                    try
                    {
                        ...
                        if (orderdt.Rows.Count > 0)
                        {
                            ...
                            if (dt.Rows.Count == 0)
                            {
                                ...
                                dt.Columns.Add("parter", System.Type.GetType("System.String"));
                                ...
                                if (dt.Rows.Count > 0)
                                {
                                    ...
                                    try
                                    {
                                        DataTable DTPaytime = SQLHelper.ExecuteDt("SELECT top 1 paytime,alipayorderid FROM alipayManyLog where parter ='" + userid.Text + "' order by paytime desc");
                                    ...
                                    if (paylist.Length > 0)
                                    {
                                        ...
                                        sqlhelp.RunProc("proc_orderbank_alipayManyLog", para, out reader);
                                        while (reader.Read())
                                        {
                                            string strorder = DateTime.Now.ToString() + "，商户：" + reader[4].ToString() + "，订单：" + reader[0].ToString() + "金额：" + reader[1].ToString() + ",支付宝：" + reader[2].ToString() + "," + reader[3].ToString();
        ...
        private void GetBankParameter(int i)
        {
            ...
            try
            {
                struserid = " and userid in(" + userid.Text + ")";
